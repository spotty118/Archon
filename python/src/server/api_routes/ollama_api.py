...
async def _assess_archon_compatibility_with_testing(model, instance_url: str) -> dict[str, Any]:
    ...
    return {"level": compatibility_level, "features": features, "limitations": limitations}
...
def _assess_archon_compatibility(model) -> dict[str, Any]:
    ...
    full_support_patterns = ["qwen", "llama", "mistral", "phi", "codeqwen", "codellama", "deepseek"]

    partial_support_patterns = [
        "gemma",
        "mixtral",
        "neural-chat",  # Removed 'deepseek' - it should be tested
    ]
...
async def discover_models_with_real_details(request: ModelDiscoveryAndStoreRequest) -> ModelDiscoveryResponse:
    ...
    try:
                    ...
                    for model_data in tags_data["models"]:
                        ...
                        try:
                            ...
                            if model_type == "chat":
                                ...
                                compatibility = {
                                    "level": "full",
                                    "features": ["High-quality embeddings", "Local processing"],
                                    "limitations": [],
                                }
        ...
        embedding_models_with_dims = [
            m for m in stored_models if m.get("model_type") == "embedding" and m.get("embedding_dimensions")
        ]
        logger.info(
            f"Storing {len(embedding_models_with_dims)} embedding models with dimensions: {[(m['name'], m.get('embedding_dimensions')) for m in embedding_models_with_dims]}"
        )
        ...
        for model in stored_models:
            ...
            if host not in host_status:
                host_status[host] = {"status": "online", "models_count": 0, "instance_url": model["host"]}
...
async def test_model_capabilities_endpoint(request: ModelCapabilityTestRequest) -> ModelCapabilityTestResponse:
    ...
    try:
        ...
        logger.info(
            f"Capability testing complete for {request.model_name}: {compatibility_level} support detected in {duration:.2f}s"
        )
