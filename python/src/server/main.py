...
_schema_check_cache: dict[str, bool | float | dict | None] = {"valid": None, "checked_at": 0.0}
...
async def _check_database_schema():
    ...
    checked_at = _schema_check_cache.get("checked_at", 0.0)
    if isinstance(checked_at, int | float) and _schema_check_cache["valid"] is False and current_time - checked_at < 30:
