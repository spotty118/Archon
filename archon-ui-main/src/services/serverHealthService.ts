...
class ServerHealthService {
  ...
  handleImmediateDisconnect() {
    console.warn('🏥 [Health] Immediate disconnect triggered');
    this.isConnected = false;
    this.missedChecks = this.maxMissedChecks; // Set to max to ensure disconnect screen shows
    
    if (this.disconnectScreenEnabled && this.callbacks) {
      console.warn('🏥 [Health] Triggering disconnect screen immediately');
      this.callbacks.onDisconnected();
    }
  }
  ...
  handleConnectionReconnect() {
    console.warn('🏥 [Health] Connection reconnected, resetting missed checks');
    this.missedChecks = 0;
