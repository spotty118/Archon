...
import { Check, Save, Loader, ChevronDown, ChevronUp, Zap, Database, Trash2 } from 'lucide-react';
...
export const RAGSettings = ({
  ...
  useEffect(() => {
    const loadApiCredentials = async () => {
      try {
        ...
        // Failed to load API credentials for status checking
      }
    };
  ...
  const reloadApiCredentials = async () => {
    try {
      ...
      // Failed to reload API credentials
    }
  };
  ...
  }, [ragSettings.LLM_PROVIDER]); // Only depend on LLM_PROVIDER changes, reloadApiCredentials is stable
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ragSettings.LLM_PROVIDER]); // Only restart interval if provider changes, reloadApiCredentials is stable
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [apiCredentials]); // Test when credentials change, providerConnectionStatus is updated internally
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (instanceStatus?.is_healthy) {
          ...
          showToast(`${instanceName} connection successful`, 'success');
      ...
      if (error && typeof error === 'object' && 'name' in error && error.name === 'AbortError') {
        ...
        showToast(`${instanceName} connection failed: ${errorMessage}`, 'error');
      }
    }
  };
  ...
  const handleDeleteLLMInstance = () => {
    ...
    if (window.confirm('Are you sure you want to delete the current LLM instance configuration?')) {
      ...
      const { ollama_llm_url, ...remainingSettings } = updatedSettings;
      setRagSettings(remainingSettings);
  ...
  const handleDeleteEmbeddingInstance = () => {
    ...
    if (window.confirm('Are you sure you want to delete the current embedding instance configuration?')) {
      ...
      // Clear related RAG settings
      const updatedSettings = { ...ragSettings };
      const { ollama_embedding_url, embedding_model, ...remainingSettings } = updatedSettings;
      setRagSettings(remainingSettings);
  ...
  const fetchOllamaMetrics = useCallback(async () => {
    try {
      ...
      // Error fetching Ollama metrics
      setOllamaMetrics(prev => ({ ...prev, loading: false }));
    }
  }, [llmInstanceConfig.url, embeddingInstanceConfig.url, llmStatus.online, embeddingStatus.online]);
  ...
  }, [ragSettings.LLM_PROVIDER, llmInstanceConfig.url, embeddingInstanceConfig.url, llmStatus.online, embeddingStatus.online, fetchOllamaMetrics]); // Only watch provider changes, not URL changes
  ...
  useEffect(() => {
    ...
    if (!hasRunInitialTestRef.current && 
      ...
      // Settings page loaded with Ollama - Testing connectivity
      ...
      if (llmInstanceConfig.url && ragSettings.LLM_BASE_URL) {
        setTimeout(() => {
          ...
          // Testing LLM instance on page load
      ...
      if (embeddingInstanceConfig.url && ragSettings.OLLAMA_EMBEDDING_URL &&
        ...
        setTimeout(() => {
          ...
          // Testing Embedding instance on page load
      ...
      setTimeout(() => {
        // Fetching Ollama metrics on page load
        fetchOllamaMetrics();
      }, 2000);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
...
function getDisplayedChatModel(ragSettings: { LLM_PROVIDER?: string; MODEL_CHOICE?: string }): string {
...
function getDisplayedEmbeddingModel(ragSettings: { LLM_PROVIDER?: string; EMBEDDING_MODEL?: string }): string {
  const provider = ragSettings.LLM_PROVIDER || 'openai';
